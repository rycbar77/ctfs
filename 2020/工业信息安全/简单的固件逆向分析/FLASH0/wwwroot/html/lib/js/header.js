...
function writeMenuProduct(items)
{
  ...
  for(var i=0; i<items[0].length; i++)
  {
    ...
    if(id=='logout')
    {
      document.write('<td align="right" width="100%" class="menuProduct" ><a href="' + items[0][i][2] + '" target="_parent"' + '>' + name + '</a></td>');
    }
    else
    {
      document.write('<td id="TD' + id + '" class="menuProduct" ><a id="A' + id + '" ' + (disabled?'style="color:Silver"':'href="' + id + '/index.htm" target="mainTop"') + '>' + name + '</a></td>');
    }
  }
  document.write('</tr></table>');
}
